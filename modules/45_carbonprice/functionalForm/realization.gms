...
*'               or to meet global net-zero CO2 targets.
...
*'                                     The (initial) carbon price in cm_startyear is chosen via cm_taxCO2_startyear. Alternatively, the (initial) carbon price in cm_peakBudgYr is chosen via cm_taxCO2_peakBudgYr.
*'                                     This value is endogenously adjusted to meet CO2 budget targets if cm_iterative_target_adj is set to 5, 7, 8 or 9.
...
*'                                     (with iterative_target_adj = 8 or 9): after cm_peakBudgYr, the global anchor trajectory increases linearly with fixed annual increase given by cm_taxCO2_IncAfterPeakBudgYr (default = 0, i.e. constant)
